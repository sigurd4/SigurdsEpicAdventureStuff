...
public class ItemMysteryPotion extends Item implements IItemSubItems
{
	...
	public ItemMysteryPotion()
	{
		...
		this.setNoRepair();
		String[] potionPrefixes = this.getPotionPrefixes();
	...
	{
		return this.getEffects(stack.getMetadata());
	}
	...
	public ItemStack onItemUseFinish(ItemStack stack, World world, EntityPlayer player)
	{
		...
		props.drinkPotion(stack.getMetadata());
	...
	public int getColorFromDamage(int meta)
	{
		...
		if(!((ProxyClient)M.proxy).potionColorMap.containsKey(Config.potionColor.get()) || !((ProxyClient)M.proxy).potionColorMap.get(Config.potionColor.get()).containsKey(Config.potionColorSimilarityThreshold.get()))
		{
			((ProxyClient)M.proxy).potionColorMap = new HashMap();
			((ProxyClient)M.proxy).potionColorMap.put(Config.potionColor.get(), new HashMap());
			((ProxyClient)M.proxy).potionColorMap.get(Config.potionColor.get()).put(Config.potionColorSimilarityThreshold.get(), new HashMap());
		}
		if(!((ProxyClient)M.proxy).potionColorMap.get(Config.potionColor.get()).get(Config.potionColorSimilarityThreshold.get()).containsKey(world))
		{
			((ProxyClient)M.proxy).potionColorMap.get(Config.potionColor.get()).get(Config.potionColorSimilarityThreshold.get()).put(world, new HashMap());
		}
		((ProxyClient)M.proxy).potionColorMap.get(Config.potionColor.get()).get(Config.potionColorSimilarityThreshold.get()).get(world).put(meta, c);
		return c.getRGB();
	}
	...
	public Color getColorFromDamage2(int meta)
	{
		...
		if(Config.potionColor.get() == EnumPotionColorMethod.RANDOMIZED_TINT || Config.potionColor.get() == EnumPotionColorMethod.RANDOMIZED_ALL)
		{
			...
			{
				if(((ProxyClient)M.proxy).potionColorMap.get(Config.potionColor.get()).containsKey(Config.potionColorSimilarityThreshold.get()))
				{
					if(((ProxyClient)M.proxy).potionColorMap.get(Config.potionColor.get()).get(Config.potionColorSimilarityThreshold.get()).containsKey(world))
					{
						if(((ProxyClient)M.proxy).potionColorMap.get(Config.potionColor.get()).get(Config.potionColorSimilarityThreshold.get()).get(world).containsKey(meta))
						{
							return ((ProxyClient)M.proxy).potionColorMap.get(Config.potionColor.get()).get(Config.potionColorSimilarityThreshold.get()).get(world).get(meta);
						}
					}
				}
			}
			...
			for(int seed = 0;; ++seed)
			{
				Color c = this.getColorFromDamage(meta, Stuff.Randomization.randSeed(seed, meta, 35234324l, seed).nextLong());
				if(seed < cs.length)
				{
					cs[seed] = c;
				}
				...
				for(int i = 1; i < meta; ++i)
				{
					...
					for(ItemStack stack : stacks)
					{
						...
						{
							Color c2 = new Color(this.getColorFromDamage(i));
	...
	public Color getColorFromDamage(int meta, long seed)
	{
		...
		{
			c = Color.getHSBColor((float)meta / this.getMaxDamage() + (float)time / 80, 1F, 1F);
			break;
		}
		case RAINBOW:
		{
			c = Color.getHSBColor((float)meta / this.getMaxDamage() + Stuff.Randomization.randSeed(worldSeed, 6576474524l, seed).nextFloat(), 1F, 1F);
			break;
		}
	...
	public String getItemStackDisplayName(ItemStack stack)
	{
		...
		s1 = PotionHelper.getPotionPrefix(stack.getMetadata() * 2);
	...
	{
		ExtendedPlayer props = ExtendedPlayer.get(player);
		if(props.knowsPotion(stack.getMetadata()))
		{
			List list1 = this.getEffects(stack);
	...
	public void getSubItems2(Item item, CreativeTabs tab, ArrayList<ItemStack> stacks)
	{
		...
		if(potionPrefixes != null)
		{
			for(int i = 0; i < potionPrefixes.length; ++i)
			{
				...
				if(potionPrefixes[i] != null)
				{
					...
					{
						if(potionPrefixes[types.get(i2)] != null)
						{
							if(potionPrefixes[i].contains(potionPrefixes[types.get(i2)]) || i == types.get(i2))
							{
								contains = true;
								break;
							}
		...
		{
			stacks.add(new ItemStack(item, 1, types.get(i)));
		}
		if(stacks.size() <= 0)
		{
