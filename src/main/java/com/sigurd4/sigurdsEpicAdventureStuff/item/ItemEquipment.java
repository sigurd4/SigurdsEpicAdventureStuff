...
public abstract class ItemEquipment extends Item implements IItemTextureVariants, IItemDynamicModel, IItemSubItemRandom
{
					...
					public final ItemTagEnum<ArmorMaterial> MATERIAL_DECO = new ItemTagEnum<ArmorMaterial>("MaterialDeco", ArmorMaterial.IRON, false)
						...
						protected ArmorMaterial NBTTagToRaw(NBTTagInt value)
						{
							...
							public void getSubItems(Item item, CreativeTabs tab, List list)
							{
								...
								for(ArmorMaterial mat1 : this.materialsBase)
								{
									stacks.add(this.generateRandomSubItem(item, mat1, null, true));
									for(ArmorMaterial mat2 : this.materialsDeco)
									{
										stacks.add(this.generateRandomSubItem(item, mat1, mat2, true));
									}
								}
								stacks = this.sortStacksAfterRarity(stacks);
								for(ItemStack stack : stacks)
								{
									if(((ItemEquipment)stack.getItem()).isCursed(stack))
									{
										this.applyCurse(stack);
									}
								}
								list.addAll(stacks);
							...
							private ArrayList<ItemStack> sortStacksAfterRarity(ArrayList<ItemStack> stacks)
							{
							...
							@Override
							public ItemStack getRandomSubItem(Item item)
							{
								ArmorMaterial mat1 = this.materialsBase[Item.itemRand.nextInt(this.materialsBase.length)];
								int i = Item.itemRand.nextInt(this.materialsDeco.length + 1);
								ArmorMaterial mat2 = i < this.materialsDeco.length ? this.materialsDeco[i] : null;
								return this.generateRandomSubItem(item, mat1, mat2, true);
							}

							public ItemStack generateRandomSubItem(Item item, ArmorMaterial mat1, ArmorMaterial mat2, boolean setCursedNow)
							{
								ItemStack stack = new ItemStack(item);
								this.MATERIAL_BASE.set(stack, mat1);
								if(mat2 == null)
								{
								...
								{
									((ItemEquipment)stack.getItem()).MATERIAL_DECO.set(stack, mat2);
								}
							...
							private void applyCurse(ItemStack stack)
							{
